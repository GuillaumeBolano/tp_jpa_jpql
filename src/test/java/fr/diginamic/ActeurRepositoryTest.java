...
public class ActeurRepositoryTest {
	...
	public void testExtraireActeursParFilmParuAnnee() {
		TypedQuery<Acteur> query = em.createQuery("SELECT a FROM Acteur a " +
						"JOIN a.roles r " +
						"JOIN r.film f WHERE f.annee = :annee", Acteur.class);
	...
	public void testExtraireParRealisateurEntreAnnee() {
		TypedQuery<Acteur> query = em.createQuery("SELECT a FROM Acteur a " +
						...
						"WHERE real.identite = :identite " +
						"AND f.annee BETWEEN :anneeMin AND :anneeMax", Acteur.class);
		query.setParameter("identite", "Ridley Scott");
		query.setParameter("anneeMin", 2010);
		query.setParameter("anneeMax", 2020);
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireRealisateursParActeur() {
		TypedQuery<Realisateur> query = em.createQuery("SELECT real FROM Realisateur real " +
						"JOIN real.films f " +
						"JOIN f.roles r " +
						"JOIN r.acteur a " +
						"WHERE a.identite = :identite", Realisateur.class);
		query.setParameter("identite", "Brad Pitt");
